...
def fetch_sourcetable(host: str, port: int, user: str = None, password: str = None, ntrip_version: str = "1.0") -> None:
    """Connects to the caster and downloads/prints the mountpoints."""
    print(f"Fetching sourcetable from {host}:{port}...")
    try:
        ...
        if ntrip_version == "2.0":
            ...
            headers.append("GET / HTTP/1.0")
            
        headers.append("User-Agent: NTRIP AntigravityClient/1.0")
        ...
        if "200" not in headers and "ICY" not in headers:
            print("Failed to retrieve sourcetable. Headers response:")
            print(headers)
            return
            
        print("\nAvailable Mountpoints (Top 25):")
        print(f"{'Mountpoint':<15} | {'Format':<10} | {'Details':<20} | {'Location':<25}")
        print("-" * 80)
...
def main():
    ...
    parser.add_argument("--host", default="rtk2go.com", help="NTRIP caster hostname/IP (default: rtk2go.com)")
    parser.add_argument("--port", type=int, default=2101, help="NTRIP caster port (default: 2101)")
    parser.add_argument("--mountpoint", help="NTRIP mountpoint name (if omitted, lists the sourcetable)")
    parser.add_argument("--user", default=None, help="NTRIP username")
    parser.add_argument("--password", default=None, help="NTRIP password")
    parser.add_argument("--ntrip-version", choices=["1.0", "2.0"], default="1.0", help="NTRIP protocol version (default: 1.0)")
    parser.add_argument("--duration", type=int, default=15, help="Time to stream and parse messages in seconds (default: 15)")
    parser.add_argument("--lat", type=float, default=None, help="Latitude for NMEA GGA sentence (must specify both lat and lon to send GGA)")
    parser.add_argument("--lon", type=float, default=None, help="Longitude for NMEA GGA sentence (must specify both lat and lon to send GGA)")
    parser.add_argument("--alt", type=float, default=100.0, help="Elevation (meters) for NMEA GGA sentence (default: 100.0)")
    parser.add_argument("--verbose", action="store_true", help="Print messages as they are received")
    
    args = parser.parse_args()
    
    if not args.mountpoint:
        fetch_sourcetable(args.host, args.port, args.user, args.password, args.ntrip_version)
        return
    ...
    try:
        ...
        headers = []
        if args.ntrip_version == "2.0":
            headers.append(f"GET /{args.mountpoint} HTTP/1.1")
            headers.append(f"Host: {args.host}:{args.port}")
            headers.append("Ntrip-Version: 2.0")
        else:
            headers.append(f"GET /{args.mountpoint} HTTP/1.0")
        ...
        if "200" not in header_text and "ICY" not in header_text:
            print("\nError: Handshake failed. Caster response headers:")
            print(header_text)
            s.close()
            return
            
        # Only send NMEA GGA sentence if coordinates were specified on CLI
        send_gga = args.lat is not None and args.lon is not None
        if send_gga:
        ...
        print(f"Streaming data. Parsing for {args.duration} seconds (Press Ctrl+C to stop)...")
        print("-" * 50)
        ...
        while True:
            ...
            now = time.time()
            if send_gga and now - last_gga_time >= gga_interval:
                try:
                    s.sendall(make_gga(args.lat, args.lon, args.alt).encode('utf-8'))
                    last_gga_time = now
                except socket.error:
                    pass
    ...
    if total_messages == 0:
        print("No RTCM messages were received. Verify the mountpoint is active and does not require private auth.")
        print("=" * 70)
        return
        
    print("Received Message Types:")
    for ident, count in sorted(msg_counts.items(), key=lambda x: int(x[0]) if x[0].isdigit() else 9999):
        name = RTCM_MSG_NAMES.get(ident, "Unknown")
        print(f"  - Message {ident:<4} ({name:<25}): {count} messages")
    print("-" * 70)
    
    if not sat_signals:
        print("No MSM (Multiple Signal Messages) parsed. Satellite/Band breakdown is only available for MSM messages.")
        print("=" * 70)
        return
    ...
    for gnss, sats in sorted(sat_signals.items()):
        ...
        sorted_sats = sorted(sats.items(), key=lambda x: (x[0][:1], int(x[0][1:]) if x[0][1:].isdigit() else 999))
